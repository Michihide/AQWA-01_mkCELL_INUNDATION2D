# AQWA-INUNDATION2D メッシュ作成ツール（legacy — 旧ネイティブパイプライン）

> **注意:** 2025-08-17 以降、メインのメッシュ生成はリポジトリルートの [`tool/`](../tool/)（Gmsh 版）です。
> 本ディレクトリは統合前の旧実装の退避先です。ロールバック手順は [ルート README](../README.md) を参照。
